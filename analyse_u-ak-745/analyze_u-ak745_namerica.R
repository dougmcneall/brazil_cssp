...
#read data
frac = read.table('namerica_ed.txt', header = FALSE, skip = 1)


lhs = read.table('lhs_u-ak745.txt', header = TRUE)

#forest_frac = frac[, 2]
forest_frac = c(frac, recursive = TRUE)
...
# Histogram of forest fraction
pdf(file = 'namerica_needleleaf_forest_fraction_hist.pdf', width = 6, height = 4)
#dev.new()
par(fg = 'white', las = 1)
hist(forest_frac, col = 'grey', axes = FALSE, main = '', xlab = 'Needleleaf Forest Fraction',
     xlim = c(0,0.16))
axis(1, col = 'black')
axis(2, col = 'black')
#legend('topright',legend = 'LCCCI observed forest fraction', pch = '|',
#       col = 'red', bty = 'n', pt.cex = 1.5, text.col = 'black')
rug(0.16, lwd = 2, col = 'red')
dev.off()


# Lightweight plotting of the entire ensemble
# dev.new(width = 8, height = 10)
pdf(file = 'namerica_param_vs_tffraction.pdf', width = 8, height = 7)
...
pdf(file = 'namerica_xval.pdf', width = 5, height = 10)
plot(fit)
dev.off()
...
pdf(file = 'namerica_oaat.pdf', width = 8, height = 10)
...
pdf(file = 'namerica_dot.pdf', width = 5, height = 10)
...
ix.kept  = which(abs( pred.unif$mean - obs) <= thres)
ix.rejected = which(abs( pred.unif$mean - obs) > thres)

# how much space is rejected when we remove those samples without a functioning
# forest?
prop.rejected = length(ix.rejected) / nsamp

kept = X.unif[ix.kept, ix.top]

colnames(kept) = colnames(lhs)[ix.top]

pdf(file = 'namerica_needleleaf_kept_hists.pdf', width = 8, height = 6)
...
pdf(file = 'namerica_needleleaf_density.pdf', width = 8, height = 8)
par(oma = c(0,0,0,3))
test = pairs(X.unif[ix.kept, ix.top],
...
pdf(file = 'namerica_needleleaf_fast.pdf', width = 12, height = 6)
par(las = 2, mar = c(10,4,2,1))
plot(fast, ylim = c(0,0.1))
dev.off()
